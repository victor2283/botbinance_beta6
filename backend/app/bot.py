...
class BotBinance:
    def __init__(self, symbol: str, asset_primary:str, asset_secundary: str,  mode_Soft: bool = True, interval: str = "1m", limit: int=300, sPd:float =9, mPd:float=18, lPd:float=54, perc_binance:float = 0.166, perc_stopSide:float=0.038, perc_priceSide:float=0.018, nbdevup:float=2.0, nbdevdn:float=2.0, api_key: str="" , api_secret: str = ""):
    ...
    def new_order(self, side: str,  type: str, quantity: float = 0, price: float = 0, stopPrice: float = 0, mode: bool = True):
    ...
    def exchange_info(self, symbol: str= ""):
        ...
        if symbol=="":
            return self._request(endpoint="exchange_info")    
        else:
            return self._request(endpoint="exchange_info", parameters=params)        
    ...
    def SMA(self, closes, timeperiod:float = 20):
        timeperiod_int= int(timeperiod)
        sma = []
        for i in range(len(closes)):
            if i < timeperiod_int - 1:
                sma.append(None)  # No hay suficientes datos para calcular la SMA
            else:
                sma.append(sum(closes[i-timeperiod_int+1:i+1]) / timeperiod_int)
        return sma
    ...
    def RSI(self, closes, timeperiod:float = 20):
        ...
        avg_gain = gains.rolling(window=timeperiod_int, min_periods=1).mean()
        avg_loss = losses.rolling(window=timeperiod_int, min_periods=1).mean()
    ...
    def calculate_ema(self, series, timeperiod:float = 20):
        timeperiod_int= int(timeperiod)
        return series.ewm(span=timeperiod_int, adjust=False).mean()
    ...
    def BBANDS(self, closes, timeperiod: float = 20, nbdevup: float = 2, nbdevdn: float = 2):
        timeperiod_int = int(timeperiod)
        rolling_mean = closes.rolling(window=timeperiod_int).mean()
        rolling_std = closes.rolling(window=timeperiod_int).std()
    ...
    def MFI(self, highs,  lows, closes, volume,  timeperiod:float = 14):
        ...
        positive_mf = pd.Series(positive_flow).rolling(window=timeperiod_int, min_periods=1).sum()
        negative_mf = pd.Series(negative_flow).rolling(window=timeperiod_int, min_periods=1).sum()
    ...
    def update_data(self):
        ...
        elif len(order_trade)>0 and  order_trade['isBuyer'] == False: 
            price_buy = 0
            if order_trade['orderId'] != self.last_order_tradeId:
                self.sTrade = self.sTrade + 1
                self.last_order_tradeId= order_trade['orderId']
        ...
        priceBuy= priceSide
        if fiat > 0 and priceBuy > 0 and price_min_sell > 0:
        ...
        if orderId !=0:
            ...
            if ( self.last_trend=="neutral" or  self.last_trend=="consolidation")  and ((aux_side == "SELL" and trend == "up" and priceSell > aux_price) or (aux_side == "BUY" and trend== "down" and  priceBuy < aux_price)):
            ...
            if price_buy > 0 and (quantity > price_min_sell and fiat < price_min_buy):
                ...
                if self.last_trend !="up" and trend=="down" and alert_mfi=="down" and alert_rsi=="down" and (perc_stop_loss > perc_binance) and alert_band=="down":    
                ...
                if trend=="up" and self.last_alert_macd !="down_div" and (alert_mfi== "up" or alert_rsi== "up") and alert_band=="up": 
        ...
        self.last_alert_macd= alert_macd
